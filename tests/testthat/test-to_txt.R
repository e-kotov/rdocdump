...
test_that("rdd_to_txt outputs only documentation when content is 'docs'", {
  ...
  # Check that the output includes the DESCRIPTION and Rd
...
test_that(
  paste(
    "rdd_to_txt keeps both tar.gz archive and extracted files",
    "when keep_files is 'both'"
  ),
  {
    skip_on_cran()
    skip_if_offline()
    ...
    # For the extracted package: resolve_pkg_path creates an extraction
    # directory as file.path(cache_dir, <pkgname>, <version>).
    # For package "ini", we expect a subdirectory named "ini" under cache_dir.
...
test_that(
  "rdd_to_txt fetches a specific package version when 'version' is provided",
  {
    skip_on_cran()
    skip_if_offline()
...
test_that(
  "rdd_to_txt sets force_fetch=TRUE internally whenever 'version' is provided",
  {
...
test_that(
  "rdd_to_txt passes through force_fetch unchanged when 'version' is NULL",
  {
    calls <- new.env(parent = emptyenv())
    calls$resolve_force_fetch <- NULL
    calls$rdd_force_fetch <- NULL
    ...
    # Case A: user passes force_fetch = FALSE, version = NULL
    # internal should be FALSE
    out_A <- rdd_to_txt(
      "ini",
      version = NULL,
      force_fetch = FALSE,
    ...
    # Case B: user passes force_fetch = TRUE, version = NULL
